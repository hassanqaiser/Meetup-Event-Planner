  ...
  .controller('createEventCtrl', ['$scope', '$http', 'FOURSQURE_API', '$state', 'events', 'distanceCalculator', function ($scope, $http, FOURSQURE_API, $state, events, distanceCalculator) {
    ...
    vm.removeGuest = function (guest) {
	  ...
	  $scope.$watch('vm.location', function (newValue, oldValue) {
		...
		$scope.$watch('vm.startDate', checkDates);
		$scope.$watch('vm.endDate', checkDates);
	    ...
	    if (vm.startDate) {
				$scope.createEventForm.startDate.$setValidity("startBeforeNow", startDate >= now);
	    }
			if (vm.endDate) {
				$scope.createEventForm.endDate.$setValidity("endBeforeOrOnStart", endDate > startDate);
			}
		}
